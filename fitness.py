from functions import random_vector, random_vectors
import numpy as np
        ...
        if model == None:
            ...
            for j in range(lhv_size):
                ...
                for i in range(input_size):
                    ...
                    pred_prob = 1/2 + 1/4 * np.sign(mdot(input, lhv[0], M.value[0])) * np.sign(mdot(input, lhv[1], M.value[0])) \
                    ...
                    true_probs[lhv_size*j+i,:] = true
                    pred_probs[lhv_size*j+i,:] = pred

            res = KL_distance(true_probs, pred_probs)/(lhv_size*input_size)
        
        else:
            xarray = np.concatenate((np.tile(inputs, lhv_size), np.repeat(lhvs, input_size, axis=0)), axis=1)
            ...
            if slice == 'Alice_1':
                ...
                true = np.array([yarray[:,0], 1-yarray[:,0]])
            res = KL_distance(true, pred)/(lhv_size*input_size)
        return -res
